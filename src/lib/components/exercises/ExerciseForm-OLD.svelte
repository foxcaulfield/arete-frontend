<script lang="ts">
	...
	function getExerciseTypeDescription(type: string): string {
		const descriptions: Record<string, string> = {
			FILL_IN_THE_BLANK: "Users type the correct answer",
			CHOICE_SINGLE: "Users select one correct answer from options",
		};
	...
	<div class="max-w-5xl mx-auto">
		...
		<div class="mb-8 flex items-center justify-between">
			<div>
				...
				{/if}
				<p class="text-surface-400 mt-2">Step by step, create a great exercise for your learners</p>
			</div>
		</div>

		<!-- Error Alert -->
		{#if generalError}
			<div class="mb-6 p-4 bg-error-900 border border-error-800 rounded-lg flex items-start gap-3 animate-pulse">
				...
				<div class="text-sm text-error-400 font-medium">{generalError}</div>
			</div>
		{/if}

		<!-- Tab Navigation -->
		<div class="mb-6 flex gap-2 border-b border-surface-700">
			<button
			...
			>
				<span class="flex items-center gap-2">
					<span class="text-lg">📝</span> Basic Info
				</span>
			</button>
			<button
			...
			>
				<span class="flex items-center gap-2">
					<span class="text-lg">⚙️</span> Advanced
				</span>
			</button>
		</div>
			action={formAction}
